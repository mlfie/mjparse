...
function ajaxRuncher(requestName,type,url,json,func) {
    ...
    Msg.debug("ajaxRuncher " + requestName + "リクエスト",
           "type=" + type +"<br>"
           + "url=" + url +"<br>"
    ...
    $.ajax({
        ...
        success: function (data, textStatus, xhr) {
            ...
            Msg.debug("ajaxRuncher " + requestName +  "成功:レスポンス",json2html(eval(data)));
            func(data);
        },
        error: function (data) {
            $.mobile.hidePageLoadingMsg();
            Msg.error(requestName + "に失敗しました。:" + data.status , data.responseText);
            Msg.debug("ajaxRuncher " + requestName + "失敗:レスポンス",data.responseText);
        }
    });
}
...
function btnCapturePhoto() {
    Msg.debug("btnCapturePhoto","start");
...
function btnSelectPhoto() {
    Msg.debug("btnSelectPhoto","start");
    navigator.camera.getPicture(
        cameraSuccess,
        cameraFail,
...
function cameraSuccess(imageData){
    Msg.debug("cameraSuccess","Photo Image(base64)=" + imageData);
...
function btnCalcPoint(){
    ...
    if(Photo.isEmpty()){
        ...
        Msg.debug("btnCalcPoint","img_url is empty");
        return ;
